...
username = os.environ["CLOUD189_USERNAME"]
password = os.environ["CLOUD189_PASSWD"]
...
#            sc = requests.post(scurl,data=data)
    ...
    if ("errorCode" in response.text):
        ...
        description = response.json()['description']
        print(f"抽奖获得{description}")
        spcs = f"第二次抽奖获得{description}"
    if(SCKEY != ""):
        data = {
                "text" : texts,
                "desp" : '{}\n{}\n{}'.format(sps,spcf,spcs)
                }
        sc = requests.post(scurl, data=data)
